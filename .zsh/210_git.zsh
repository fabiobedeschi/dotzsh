...
alias gbda='git branch --no-color --merged | command grep -vE "^([+*]|\s*($(git_main_branch)|$(git_develop_branch)|docs|$(git_current_branch)|sandbox)\s*$)" | command xargs git branch -d 2>/dev/null'
alias gbD='git branch -D'
...
alias gdor='git diff origin/$(current_branch)'
